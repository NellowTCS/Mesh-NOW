#!/usr/bin/env python3
"""
Manages firmware release versioning for the Mesh-NOW web flasher.
(Based off of https://github.com/TalismanDesign/PocketMage_PDA/blob/main/Code/WebFlasher/webflasher.py)
"""
...
class FirmwareReleaseManager:
    """Coordinate the web flasher firmware directory against a live gh-pages branch."""

    def __init__(self, workspace: Path, keep_dev_count: int = 2):
        self.workspace = workspace
        self.webflash_dir = workspace / "webflash"
        self.firmware_dir = self.webflash_dir / "firmware"
    ...
    def get_version_dirs(self, base_dir: Path, pattern: str = "*") -> list[str]:
        """Return version directory names matching the pattern, sorted."""
    ...
    def preserve_from_gh_pages(self, gh_pages_dir: Path, current_version: str):
        """Copy prior releases and dev builds from gh-pages into the workspace."""
        ...
        if dev_dirs:
            if self.is_dev_version(current_version):
                ...
                print(
                    f"  Current build is dev, keeping {keep_count} previous dev builds"
                )
            else:
    ...
    def _safe_version_sort_key(self, version_str: str):
        """
        Build a sort key for version strings.
        Tries PEP 440 parsing first, falls back to lowercased string comparison.
        """
        try:
    ...
    def rebuild_manifest_index(self):
        """Rebuild manifest-index.json from the actual firmware directories."""
        ...
        manifest_path = self.webflash_dir / "manifest-index.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage Mesh-NOW web flasher firmware releases"
    )
    ...
    parser.add_argument(
        "--gh-pages-dir", required=True, help="Temp directory for gh-pages clone"
    )
    parser.add_argument(
        "--keep-dev", type=int, default=2, help="Number of dev builds to keep"
    )
