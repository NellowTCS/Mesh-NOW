...
source: "https://NellowTCS.github.io/Mesh-NOW/guide/configuration/"
path: /guide/configuration/
updated: 2026-09-15
okf:
  generated_by: "@docmd/plugin-okf"
  generated_at: "2026-09-15T19:19:08.879Z"
---
---
title: "Configuration"
description: "All compile-time constants, tuning knobs, and runtime settings."
...
Mesh-NOW is configured through compile-time tokens and runtime API calls. Runtime-tunable values (beacon interval, retry and ACK timings, peer expiry) are Kconfig options under the `Mesh-NOW` menu, exposed through `idf.py menuconfig` as `CONFIG_MESH_NOW_*`. Structural limits that affect the wire format or ABI are hard `#define`s in `mesh_now.h`.
...
```c
// Optional: peers are auto-discovered via beacons
...
### Increase Range (Higher Hop Limit)
...
A higher hop limit reaches farther but costs airtime. Every hop re-broadcasts the message to all neighbors.
::: /callout
...
Faster beacons mean quicker peer discovery at the cost of more airtime.
...
Each peer entry uses ~32 bytes of RAM (address, activity flag, 8-byte timestamp, name). 40 peers use ~1280 bytes.
...
ESP-NOW frames cap at 250 bytes. A max-length 128-byte payload already encodes to 170 bytes on the wire, 198 when encrypted (32-byte header included). Encryption always adds 28 bytes (12-byte nonce plus 16-byte auth tag). Raising `MAX_MESH_MESSAGE_LEN` much past 180 pushes encrypted frames over the cap; treat the library as tuned for short chat frames, not bulk transfer.
::: /callout
